...
const guides: Guide[] = [
  {
    ...
    summary:
      "JWTs are a compact, URL-safe way to represent claims between two parties. They're the standard for stateless authentication in modern APIs.",
  ...
  },
  {
    ...
    category: 'Backend',
    summary:
      'Rate limiting controls how many requests a client can make in a given time window. Common algorithms include fixed window, sliding window, and token bucket.',
  ...
  },
  {
    ...
    category: 'DevOps',
    summary:
      'Docker packages applications into isolated containers that include everything needed to run — code, runtime, libraries. It ensures consistency across environments.',
  ...
  },
  {
    ...
    category: 'DevOps',
    summary:
      "Kubernetes orchestrates containerized workloads — handling deployment, scaling, networking, and self-healing. It's the industry standard for running production services.",
  ...
  },
  {
    ...
    category: 'Database',
    summary:
      'Understanding EXPLAIN plans, index types (B-tree, GIN, GiST), and query patterns is critical for building performant database-driven applications.',
  ...
  },
  {
    ...
    summary:
      'Observability means understanding what your system is doing from the outside — through logs, metrics, and traces. OpenTelemetry is the emerging standard.',
    ...
    resources: [
      ...
      },
      {
        ...
        url: 'https://www.jaegertracing.io/docs/1.49/architecture/',
        source: 'Jaeger',
      },
      {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-5xl px-6 py-12">
        <Link
          href="/os"
          className="inline-flex items-center gap-2 font-serif text-[11px] uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors mb-10"
        >
        ...
        <div className="mb-10">
          ...
          <h1 className="text-3xl font-medium tracking-tight text-foreground md:text-4xl font-serif mb-2">
            Know the fundamentals.
          </h1>
          <p className="text-sm text-muted-foreground max-w-xl">
            We don't teach — we orient. Each guide explains what a technology is, why it matters in
            real work, and links you to the best resources on the web.
          </p>
        </div>
          ...
          <div className="flex items-center gap-1.5">
            {categories.map((c) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-sm text-[10px] font-serif uppercase tracking-widest transition-colors ${selectedCat === c ? 'bg-foreground text-background' : 'text-muted-foreground hover:text-foreground'}`}
              >
                {c}
              </button>
            ))}
              ...
              >
                ...
                {isOpen && (
                  <div className="border-t border-border p-6 bg-secondary/10">
                    {/* Why it matters */}
                    <div className="mb-8">
                      <h4 className="font-serif text-[10px] uppercase tracking-widest text-[#a86f44] mb-3">
                        Why this matters
                      </h4>
                    ...
                    {/* Resources */}
                    <div className="mb-8">
                      <h4 className="font-serif text-[10px] uppercase tracking-widest text-muted-foreground mb-4">
                      ...
                      <div className="space-y-2.5">
                        {guide.resources.map((r) => (
                          <a
                            ...
                            className="flex items-center justify-between p-4 rounded-sm border border-border bg-card hover:border-muted-foreground/30 transition-colors group/link"
                          >
                            <div className="flex items-center gap-3">
                              <BookOpen size={14} className="text-muted-foreground shrink-0" />
                              <div>
                                <p className="text-xs font-medium text-foreground group-hover/link:text-[#a86f44] transition-colors">
                                  {r.label}
                                </p>
                                <p className="font-serif text-[9px] text-muted-foreground/60 uppercase tracking-widest">
                                  {r.source}
                                </p>
                              </div>
                            </div>
                            <ExternalLink
                              size={14}
                              className="text-muted-foreground/30 group-hover/link:text-muted-foreground shrink-0"
                            />
                          </a>
                        ))}
                      </div>
                    ...
                    {guide.relatedScenarios.length > 0 && (
                      <div>
                        <h4 className="font-serif text-[10px] uppercase tracking-widest text-muted-foreground mb-3">
                          Practice this in
                        </h4>
                        <div className="flex items-center gap-3">
                          {guide.relatedScenarios.map((id) => (
                            <Link
                              key={id}
                              href="/scenarios"
                              className="px-3 py-1.5 rounded-sm border border-border bg-card font-mono text-[10px] text-muted-foreground hover:text-foreground hover:border-muted-foreground/30 transition-colors"
                            >
                              {id} →
                            </Link>
                          ))}
