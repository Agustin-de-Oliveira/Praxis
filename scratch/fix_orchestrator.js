...
export default function DesktopOrchestrator({ scenario, initialProgress }: DesktopOrchestratorProps) {
  ...
  const handleCodeChange = useCallback((path, code) => {
    updateCodeFile(path, code)
  }, [updateCodeFile])

  const handleOpenProgram = useCallback((id) => {
    openWindow(id)
  }, [openWindow])
    ...
    <div className="h-screen w-screen bg-[#050505] flex items-center justify-center overflow-hidden p-10">
      ...
      >
        <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 0 }}>
          <Dithering style={{ height: "100%", width: "100%" }} colorBack={osTheme === "obsidian" ? "hsla(0,0%,2%,1)" : "hsla(220,10%,5%,1)"} colorFront={osTheme === "obsidian" ? "hsl(25,15%,8%)" : "hsl(210,10%,15%)"} shape={bgShape} pxSize={3} speed={0.1} />
        </div>
        ...
        <div className={\`h-12 backdrop-blur-2xl bg-black/60 flex items-center justify-between px-3 shrink-0 relative \${taskbarPosition === "top" ? "border-b" : "border-t"} border-white/[0.08]\`} style={{ zIndex: 9999 }}>
          <div className="flex items-center gap-1.5 h-full">
            ...
            {PROGRAMS.map(prog => {
              ...
              return (
                <button key={prog.id} onClick={() => openWindow(prog.id)} className={\`relative flex items-center justify-center w-10 h-10 rounded-md transition-all \${isFocused ? "bg-white/10 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.05)] border border-white/10" : isOpen ? "bg-white/[0.03] hover:bg-white/[0.07]" : "hover:bg-white/[0.02] grayscale opacity-40"}\`}>
...
fs.writeFileSync('l:/programacion/proyectos/Praxis/components/scenario/desktop-orchestrator.tsx', content, 'utf8');
fs.writeFileSync('l:/programacion/proyectos/Praxis/scratch/fix_orchestrator.js', content, 'utf8');
