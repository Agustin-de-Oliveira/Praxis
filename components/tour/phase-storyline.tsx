...
export default function PhaseStoryline({ onContinue }: PhaseStorylineProps) {
  ...
  const getMember = (handle: string) => {
    return SCN008_TEAM.find(t => t.handle === handle) || {
      name: "Unknown",
      handle: "unknown",
      role: "Engineer",
  ...
  const handleUserSelect = (option: string) => {
    ...
    const typeTimer = setInterval(() => {
      if (charIdx < chars.length) {
        ...
        setTimeout(() => {
          setVisibleMessages(prev => [...prev, {
            user: { name: "You", color: "bg-[#a86f44]/20 border-[#a86f44]/40", textColor: "text-[#a86f44]", handle: "you" },
            time: "9:06 AM",
            text: option
          }])
    ...
    >
      ...
      <div className="rounded-sm border border-[#171717] bg-[#0A0A0A] overflow-hidden mb-8 shadow-2xl">
        ...
        <div className="px-5 py-3 border-b border-[#171717] bg-[#0F0F0F] flex items-center justify-between">
          ...
          <span className="font-mono text-[9px] text-white/20 uppercase tracking-tighter">Slack · Active Now</span>
        </div>
        ...
        >
          <AnimatePresence mode="popLayout">
            {visibleMessages.map((msg, i) => (
              ...
              >
                {/* Avatar */}
                <div className={`w-9 h-9 rounded-sm border ${msg.user.color} flex items-center justify-center font-mono text-xs font-bold ${msg.user.textColor} shrink-0`}>
                  {msg.user.name === "You" ? "ME" : msg.user.name.split(' ').map((n: string) => n[0]).join('')}
                </div>
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-sm text-white/50 leading-relaxed break-words">
                    {msg.text.split(/(@\w+|`.*?`)/g).map((part: string, idx: number) => {
                      if (part.startsWith('@')) return <span key={idx} className="text-[#a86f44] font-medium">{part}</span>
                      if (part.startsWith('`')) return <code key={idx} className="px-1.5 py-0.5 rounded-sm bg-white/5 border border-white/10 text-[11px] font-mono text-white/80">{part.slice(1, -1)}</code>
                      return part
                    })}
                  </div>
                  ...
                  {msg.isLink && (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-white font-medium">TICK-042: Add User Profile Endpoint</p>
                      </div>
                      <ArrowRight size={14} className="text-[#a86f44] group-hover:translate-x-0.5 transition-transform" />
                    </motion.button>
                  )}
                </div>
              </motion.div>
            ...
            {isTyping && (
              <motion.div 
                ...
                className="flex items-start gap-4"
              >
                <div className={`w-9 h-9 rounded-sm border ${sarah.color} flex items-center justify-center shrink-0`}>
                ...
                <div className="mt-2 text-[10px] font-mono text-white/20 uppercase tracking-widest animate-pulse">Sarah is typing...</div>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      <AnimatePresence>
        {showOptions && (
          <motion.div 
            ...
            className="space-y-2"
          >
            <p className="font-mono text-[9px] uppercase tracking-widest text-white/25 mb-3 text-center">Select your response</p>
