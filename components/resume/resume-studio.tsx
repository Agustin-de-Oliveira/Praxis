            ...
            {selectedBg && (
              ...
              >
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="font-serif text-base italic text-[#3d3429]">
                    {bgData?.label}
                  </span>
                </div>
              ...
              >
                ...
                {selectedExp && (
                  <div className="p-3 bg-white/40 border border-[#c9c2b5] rounded-sm">
                    <p className="font-mono text-[8px] uppercase tracking-[0.2em] text-[#a86f44] mb-1.5">
                      Previous Engagement
                    </p>
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className="font-mono text-[8px] text-[#6b5a46]">
                        @{' '}
                        {selectedExp === '0'
                          ? 'BitLink'
                          : selectedExp === '1'
                    ...
                    <p className="text-[10px] text-[#5c4f42] leading-tight italic">
                      {selectedExp === '0'
                        ? 'Supported React migration and documentation.'
                        : selectedExp === '1'
                          ? 'Owned API services and optimized CI/CD.'
...
export function ResumeStudio({
  ...
  const paper =
    'rounded-sm border border-[#2a2620] bg-[#ebe6dc] text-[#1a1814] shadow-[8px_16px_48px_rgba(0,0,0,0.35)]'
  ...
  return (
    <div className={`${isStandalone ? 'min-h-screen' : 'h-full flex-1'} relative overflow-auto`}>
      ...
      <div
        className={`relative z-10 transition-all duration-700 ease-in-out mx-auto px-5 ${isStandalone ? 'py-14' : 'py-8'} pb-24 ${handle.trim() ? 'max-w-[1200px]' : 'max-w-2xl'}`}
      >
        ...
        <motion.div
          layout
          className={`grid grid-cols-1 ${handle.trim() ? 'lg:grid-cols-[1fr_380px]' : 'lg:grid-cols-1'} gap-8 items-start transition-all duration-500`}
        >
          <motion.div
            layout
            className={`w-full ${handle.trim() ? 'lg:min-w-[520px]' : ''} ${paper} p-7 md:p-9 relative overflow-hidden transition-all duration-500`}
          >
            ...
            <AnimatePresence>
              {error && (
                <motion.div
                  ...
                  className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-sm"
                >
                  <p className="text-xs text-red-600 font-serif italic">{error}</p>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                ...
                >
                  <p className="font-mono text-[9px] uppercase tracking-[0.25em] text-[#a86f44] mb-2">
                    Section I — Identity
                  </p>
                  <h2 className="font-serif text-2xl text-[#1a1814] mb-3">
                    Who are you on the team roster?
                  </h2>
                  <p className="text-sm text-[#4a4339] leading-relaxed mb-9">
                    Use a handle you&apos;re comfortable seeing in code review, mail, and simulated
                    standups.
                  </p>
              ...
              {step === 2 && (
                ...
                >
                  <p className="font-mono text-[9px] uppercase tracking-[0.25em] text-[#a86f44] mb-2">
                    Section II — Trajectory
                  </p>
                  <h2 className="font-serif text-2xl text-[#1a1814] mb-3">
                    Where are you coming from?
                  </h2>
                  <p className="text-sm text-[#4a4339] leading-relaxed mb-8">
                    Calibrates scenario difficulty and narrative context—same signal a hiring
                    manager reads in a first screen.
                  </p>
                        ...
                        >
                          <div
                            className={`mt-0.5 p-2 rounded-sm border ${
                              active
                                ? 'border-[#a86f44] text-[#8b5a32]'
                                : 'border-[#c9c2b5] text-[#6b5a46]'
                            }`}
                          >
                            <Icon size={17} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2 mb-0.5">
                              <h3 className="font-mono text-[13px] font-medium text-[#1a1814]">
                                {role.title}
                              </h3>
                              {active && <Check size={16} className="text-[#a86f44] shrink-0" />}
                            </div>
                            <p className="text-xs text-[#5c4f42] leading-relaxed mb-2">
                              {role.desc}
                            </p>
