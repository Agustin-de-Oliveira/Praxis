...
import {
  ...
  Terminal, X, Circle, CheckCircle, Bot, Send,
} from "lucide-react"
...
function FileTreeNode({
  node, depth = 0, activeFile, onSelect
}: {
  node: TreeNode; depth?: number; activeFile: string; onSelect: (path: string) => void
}) {
  ...
  if (node.type === "folder") {
    return (
      <div>
        ...
        >
          {open
            ? <><ChevronDown className="w-3 h-3" /><FolderOpen className="w-3.5 h-3.5 text-amber-500/70" /></>
            : <><ChevronRight className="w-3 h-3" /><Folder className="w-3.5 h-3.5 text-amber-500/70" /></>
          }
          <span className="font-mono text-[11px]">{node.name}</span>
        </button>
        {open && node.children?.map((c, i) => (
          <FileTreeNode key={i} node={c} depth={depth + 1} activeFile={activeFile} onSelect={onSelect} />
        ))}
      </div>
    )
  }
...
export default function DynamicIDE({
  ...
  const handleTerminalCommand = (e: React.FormEvent) => {
    ...
    if (cmd === "git pull" || cmd === "git clone") {
      ...
      setTerminalLines(prev => [...prev, repoStatus === "ready" ? Object.keys(files).join("  ") : "error: directory empty or not initialized"])
    } else if (cmd === "help") {
      setTerminalLines(prev => [...prev, "Available commands: ls, git pull, git clone, clear, help"])
    } else if (cmd === "clear") {
      setTerminalLines([])
    } else {
  ...
  return (
    <div className="flex-1 flex min-h-0">
      ...
      <div className="w-52 border-r border-border bg-card shrink-0 flex flex-col">
        ...
        <div className={`flex-1 overflow-y-auto py-1 transition-opacity duration-500 ${repoStatus === "ready" ? "opacity-100" : "opacity-20 pointer-events-none"}`}>
          {tree.map((n, i) => (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center border-b border-border bg-card shrink-0 h-9">
          {repoStatus === "ready" && openTabs.map(tab => {
            ...
            return (
              ...
              >
                ...
                <button onClick={(e) => handleCloseTab(tab, e)} className="p-0.5 hover:bg-secondary rounded-sm">
          ...
          {repoStatus === "cloning" && (
            <div className="absolute inset-0 z-10 bg-[#050505]/80 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
              <div className="w-48 h-0.5 bg-white/5 rounded-full overflow-hidden">
                <motion.div initial={{ x: "-100%" }} animate={{ x: "0%" }} transition={{ duration: 2 }} className="h-full bg-blue-400" />
              </div>
              <p className="font-mono text-[9px] text-blue-400/60 uppercase tracking-[0.3em] animate-pulse">Pulling Repository...</p>
            </div>
          )}
        ...
        <div className="h-44 bg-[#050505] border-t border-border flex flex-col shrink-0">
          ...
          <div className="flex-1 overflow-auto p-3 font-mono text-[11px] space-y-1">
            {terminalLines.map((line, i) => (
              <div key={i} className={`${line.startsWith("$") ? "text-foreground" : line.startsWith("✓") ? "text-emerald-500" : "text-muted-foreground"}`}>
                {line}
              </div>
            ))}
      ...
      <div className="w-72 border-l border-border bg-card flex flex-col h-full shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Checkpoints</span>
            <span className="font-mono text-[10px] text-muted-foreground">{doneCount}/{checkpoints.length}</span>
          </div>
          <div className="w-full bg-secondary h-0.5 rounded-full mb-4">
            <div className="bg-blue-400 h-0.5 rounded-full transition-all" style={{ width: `${progress}%` }} />
          </div>
          <div className="space-y-0.5">
            {checkpoints.map(cp => {
              const done = checkpointsPassed.includes(cp.id)
              return (
                <div key={cp.id} className={`flex items-start gap-2.5 p-2 rounded-sm ${done ? "opacity-40" : "hover:bg-secondary/50"}`}>
                  {done
                    ? <CheckCircle className="w-3.5 h-3.5 text-emerald-500 mt-0.5 shrink-0" />
                    : <Circle className="w-3.5 h-3.5 text-muted-foreground mt-0.5 shrink-0" />
                  }
                  <span className={`text-xs leading-relaxed ${done ? "line-through text-muted-foreground" : "text-foreground"}`}>
                    {cp.title}
                  </span>
                </div>
              )
        ...
        <div className="p-3 border-t border-border">
            ...
            <div className="rounded-sm border border-border bg-secondary/30 p-3 mb-3">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-6 h-6 rounded-sm bg-secondary border border-border flex items-center justify-center font-mono text-[9px]">
                  {firstTeamMember[1].name.split(" ").map(n => n[0]).join("")}
                </div>
                <span className="text-[11px] font-medium text-foreground">@{firstTeamMember[0].replace(/_/g, " ")}</span>
              </div>
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">Credits</span>
            </div>
            <div className="flex items-center gap-1">
              {[1, 2, 3, 4, 5].map(i => (
                <div key={i} className={`w-1.5 h-1.5 rounded-full ${i <= 2 ? "bg-blue-400" : "bg-secondary border border-border"}`} />
              ))}
