...
const fileTree = [
  {
    ...
    children: [
      ...
      },
      {
        name: 'models',
        type: 'folder' as const,
        children: [{ name: 'user.ts', type: 'file' as const }],
      },
    ],
  },
  {
    name: 'tests',
    type: 'folder' as const,
    children: [{ name: 'oauth.test.ts', type: 'file' as const }],
  },
]
...
function FileTreeNode({ node, depth = 0 }: { node: (typeof fileTree)[0]; depth?: number }) {
  ...
  return (
    <button
      className={`w-full flex items-center gap-1.5 py-1 text-xs transition-colors ${(node as any).active ? 'text-foreground bg-secondary border-l-2 border-blue-400' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/50'}`}
    ...
    <div className="flex-1 flex min-h-0">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center border-b border-border bg-card shrink-0">
          <div className="flex items-center gap-2 px-4 py-2 border-r border-border bg-[#050505] text-foreground text-xs border-b-2 border-b-blue-400">
            ...
            <span className="font-mono text-[11px]">oauth.ts</span>
            <button className="p-0.5 hover:bg-secondary rounded-sm">
          ...
          <div className="flex items-center gap-2 px-4 py-2 border-r border-border text-muted-foreground text-xs hover:text-foreground cursor-pointer">
            <File className="w-3 h-3" />
            <span className="font-mono text-[11px]">providers.ts</span>
          </div>
        </div>
          ...
          <div className="p-4 min-w-max">
            {lines.map((line, i) => (
              <div key={i} className="flex min-h-[1.5rem]">
                <span className="w-10 text-right pr-4 text-muted-foreground/30 select-none text-[11px] leading-6">
                  {i + 1}
                </span>
                <pre className="flex-1 text-xs leading-6">
                  <code
                    ...
                    dangerouslySetInnerHTML={{
                      __html: line
                        ...
                        .replace(/>/g, '&gt;')
                        .replace(
                          /\b(import|export|from|const|function|interface|return|async|await)\b/g,
                          '<span style="color:#94A3B8">$1</span>'
                        )
                        .replace(/('.*?')/g, '<span style="color:#6b9e6b">$1</span>')
                        .replace(/(\/\/.*)/g, '<span style="color:#444">$1</span>')
                        .replace(
                          /(\|)/,
                          '<span style="color:#60a5fa;animation:pulse 1s infinite">$1</span>'
                        ),
                    }}
                  />
                </pre>
      ...
      <div className="w-72 border-l border-border bg-card flex flex-col h-full shrink-0">
        <div className="p-4 border-b border-border">
          ...
          <h2 className="text-sm font-medium text-foreground mb-1 font-mono">
        ...
        <div className="p-4 border-b border-border">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {validations.map((v) => (
              <div
                key={v.label}
                className="flex items-center gap-2 px-2 py-1.5 rounded-sm bg-secondary border border-border"
              >
                {v.status === 'passing' ? (
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="w-full bg-secondary h-0.5 rounded-full mb-4">
            <div className="bg-blue-400 h-0.5 rounded-full" style={{ width: `${progress}%` }} />
          </div>
          <div className="space-y-0.5">
            {checklist.map((item) => (
              <div
                key={item.id}
                className={`flex items-start gap-2.5 p-2 rounded-sm ${item.done ? 'opacity-40' : 'hover:bg-secondary/50'}`}
              >
                {item.done ? (
                ...
                <span
                  className={`text-xs leading-relaxed ${item.done ? 'line-through text-muted-foreground' : 'text-foreground'}`}
                >
                  {item.label}
                </span>
              </div>
        ...
        <div className="p-3 border-t border-border">
          <div className="rounded-sm border border-border bg-secondary/30 p-3 mb-3">
            ...
            <p className="font-mono text-[11px] text-muted-foreground leading-relaxed italic">
              Use Octokit for the GitHub provider. Check the access token refresh logic.
            </p>
          </div>
          <div className="flex items-center justify-between mb-2">
