...
import { Settings, Info, Database, Shield, Monitor, Layout, Palette, Waves, Type, Check, Eye, EyeOff, LayoutPanelTop, LayoutPanelLeft, MonitorCheck, Volume2, VolumeX, Radio, Speaker, Headphones, Zap } from "lucide-react"
...
interface PreferencesModalProps {
  ...
  setTaskbarPosition: (pos: "top" | "bottom") => void
  hideExtensions: boolean
  setHideExtensions: (hide: boolean) => void
  enableSounds: boolean
  setEnableSounds: (enable: boolean) => void
  bootSoundVariant: "default" | "v1" | "v2"
  setBootSoundVariant: (variant: "default" | "v1" | "v2") => void
}
...
export function PreferencesModal({ 
  ...
  hideExtensions, setHideExtensions,
  enableSounds, setEnableSounds,
  bootSoundVariant, setBootSoundVariant
}: PreferencesModalProps) {
  const [activeTab, setActiveTab] = useState<"system" | "appearance" | "audio" | "security">("system")
    ...
    <div className="flex-1 flex overflow-hidden bg-[#0A0A0A]">
      <div className="grid grid-cols-[200px_1fr] flex-1">
        ...
        <div className="bg-white/[0.02] border-r border-white/5 p-4 flex flex-col gap-1">
           ...
           >
             <Palette size={14} className={activeTab === "appearance" ? "text-[var(--accent)]" : ""} /> 
             Appearance
           </button>
           <button 
          ...
          {activeTab === "system" && (
            <div className="flex flex-col gap-8">
              <div>
                ...
                <div className="p-5 rounded-md border border-white/5 bg-white/[0.01] flex flex-col gap-4">
                  <div className="flex justify-between items-center pb-3 border-b border-white/5">
                    ...
                    <span className="font-mono text-[10px] text-white/90 uppercase select-all">{scenario ? scenario.id : "GLOBAL_WORKSTATION_01"}</span>
                  </div>
                  <div className="flex justify-between items-center pb-3 border-b border-white/5">
              ...
              <div>
                ...
                <div className={`p-5 rounded-md border flex flex-col gap-2 ${scenario ? "border-[var(--accent-muted)] bg-[var(--accent-muted)]/10" : "border-white/5 bg-white/[0.01]"}`}>
                  <p className={`font-mono text-[10px] uppercase tracking-widest font-bold ${scenario ? "text-[var(--accent)]" : "text-white/40"}`}>
                  ...
                  <p className="font-mono text-[10px] text-white/30 leading-relaxed uppercase tracking-tighter">
                    {scenario ? `Current Ticket: ${scenario.ticket.key}` : "No Active Mission Subscribed"}
                  </p>
                </div>
              </div>
            </div>
          ...
          {activeTab === "appearance" && (
            <div className="flex flex-col gap-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-3">
                  <label className="font-mono text-[9px] uppercase tracking-widest text-white/20">Status Bar</label>
                  <button 
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <LayoutPanelTop size={14} className={showTopBar ? "text-[var(--accent)]" : "text-white/20"} />
                      <span className="font-mono text-[10px] text-white/80 uppercase tracking-widest">{showTopBar ? "Visible" : "Hidden"}</span>
                    </div>
                  </button>
                </div>
                <div className="flex flex-col gap-3">
                  <label className="font-mono text-[9px] uppercase tracking-widest text-white/20">Taskbar Position</label>
                  <div className="flex gap-2">
                    <button 
              ...
              {/* Workspace Immersion */}
              <div className="flex flex-col gap-3">
                <label className="font-mono text-[9px] uppercase tracking-widest text-white/20">Workspace Immersion</label>
                <div className="grid grid-cols-2 gap-4">
                  <button 
                  ...
                  >
                    <div className="flex items-center gap-3">
                      {isWrapped ? <Monitor size={14} className="text-[var(--accent)]" /> : <Layout size={14} className="text-[var(--accent)]" />}
                      <span className="font-mono text-[10px] text-white/80 uppercase tracking-widest">{isWrapped ? "Wrapped" : "Full Page"}</span>
                    </div>
                  </button>
                  <button 
                  ...
                  >
                    <div className="flex items-center gap-3">
                      {hideExtensions ? <EyeOff size={14} className="text-[var(--accent)]" /> : <Eye size={14} className="text-[var(--accent)]" />}
                      <span className="font-mono text-[10px] text-white/80 uppercase tracking-widest">{hideExtensions ? "Simple Names" : "Show Ext."}</span>
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`p-3 rounded-full transition-all ${enableSounds ? "bg-[var(--accent-muted)] text-[var(--accent)]" : "bg-white/5 text-white/20"}`}>
                      {enableSounds ? <Volume2 size={18} /> : <VolumeX size={18} />}
                    </div>
                    <div className="text-left">
                      <p className="font-mono text-[11px] text-white/90 uppercase tracking-widest">Master Haptics</p>
                      <p className="font-mono text-[9px] text-white/30 uppercase mt-0.5">Toggle all system sound effects</p>
                    </div>
                  </div>
                  <div className="w-10 h-5 rounded-full p-1 transition-colors bg-white/10">
                    <div className={`w-3 h-3 rounded-full transition-all ${enableSounds ? "translate-x-5 bg-[var(--accent)]" : "translate-x-0 bg-white/40"}`} />
                  </div>
                </button>
              </div>
                ...
                <div className="p-5 rounded-md border border-white/5 bg-white/[0.01]">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <p className="font-mono text-[9px] uppercase tracking-widest text-white/40">Boot Sound Sequence</p>
                  </div>
                  <div className="flex gap-2">
                    {[
                ...
                <div className="p-8 border border-dashed border-white/5 rounded-md flex flex-col items-center justify-center gap-3 text-white/5">
                   ...
                   <p className="font-mono text-[9px] uppercase tracking-widest">Spatial Audio Engine Locked</p>
                </div>
            </div>
          )}
