    ...
    <div className="fixed inset-0 z-[13000] flex items-start justify-center pt-[15vh] p-4">
      ...
      >
        <div className="flex items-center px-5 py-4 border-b border-white/5 bg-white/[0.02]">
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'ArrowDown') {
                ...
                e.preventDefault()
                setActiveSearchIndex(
                  (prev) => (prev - 1 + (searchResults.length || 1)) % (searchResults.length || 1)
                )
              } else if (e.key === 'Enter') {
          ...
          />
          <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-sm border border-white/10 bg-white/5 text-[9px] text-white/40 font-mono tracking-widest uppercase ml-3">
            ESC
          </div>
        </div>
        ...
        <div className="max-h-[400px] overflow-y-auto p-2">
          {searchResults.length > 0 ? (
            searchResults.map((result, idx) => (
              ...
              >
                <div
                  className={`w-8 h-8 flex items-center justify-center rounded-sm ${idx === activeSearchIndex ? 'text-[#a86f44]' : 'text-white/40'}`}
                >
                  <result.icon size={16} />
                </div>
                <div className="flex-1 flex flex-col justify-center">
                  <p
                    className={`font-mono text-[11px] uppercase tracking-widest leading-none ${idx === activeSearchIndex ? 'text-white' : 'text-white/70'}`}
                  >
                    {result.title}
                  </p>
                  <p className="font-mono text-[9px] text-white/30 uppercase tracking-[0.2em] mt-1.5 leading-none">
                    {result.id.endsWith('.exe')
                      ? 'Application'
                      : result.id.endsWith('.txt')
                        ? 'System File'
                        : 'Tool'}
                  </p>
                </div>
            ...
            <div className="p-3">
              <p className="text-[9px] uppercase tracking-[0.2em] text-white/20 font-mono mb-3 px-1">
              ...
              <div className="grid grid-cols-2 gap-2">
                {PROGRAMS.slice(0, 4).map((p) => (
                  ...
                  >
                    ...
                    <span className="font-mono text-[10px] text-white/70 tracking-widest uppercase truncate">
                      {p.title}
                    </span>
                  </button>
                ))}
